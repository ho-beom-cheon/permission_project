...
> 현재 구현은 원본 DB와 분리된 내장 H2를 사용합니다. 아래 Oracle 문서는 과거 사전 설계이며 실행한 절차가 아닙니다. 원본 INFO 데이터의 복사·수정과 DDL 실행은 허용되지 않습니다. 현재 상태는 [이관 현황](migration/IMPLEMENTATION_STATUS.md)을 참고하세요.

## 1. 목적과 적용 범위

이 문서는 기존 인메모리 단계에서 작성한 Oracle DB 전환 사전 설계서입니다. 현재는 별도 로컬 저장과 재시작 복원이 구현되어 있습니다.
